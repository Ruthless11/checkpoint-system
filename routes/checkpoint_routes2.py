...
from models import db, VehicleLog, User
...
def dashboard():
    ...
    if company:
        query = query.filter(VehicleLog.company == company)
    if checkpoint:
    ...
    # Dropdown values
    companies = [r[0] for r in db.session.query(VehicleLog.company).distinct().order_by(VehicleLog.company)]
    checkpoints = [r[0] for r in db.session.query(VehicleLog.checkpoint).distinct().order_by(VehicleLog.checkpoint)]
    years = [r[0] for r in db.session.query(extract('year', VehicleLog.timestamp)).distinct().order_by(extract('year', VehicleLog.timestamp))]

    # Officer activity (last 15 minutes)
    ...
    if logs:
        ...
        checkpoint_totals = {}
        for log in logs:
            company_totals[log.company] = company_totals.get(log.company, 0) + log.amount_paid
    ...
    return render_template('dashboard.html',
                           ...
                           filters={"company": company, "checkpoint": checkpoint, "month": month, "year": year, "week": week, "day": day, "hour": hour, "date": specific_date},
                           companies=companies,
...
def entry():
    if current_user.role != 'officer':
        return render_template('access_denied.html', message="Only checkpoint officers can access this page.")
    ...
    if request.method == 'POST':
        log = VehicleLog(
            ...
            officer_id=current_user.id  # track the officer
        )
        db.session.add(log)
        db.session.commit()
...
def generate_report():
    ...
    query = VehicleLog.query
    if company: query = query.filter(VehicleLog.company == company)
    if checkpoint: query = query.filter(VehicleLog.checkpoint == checkpoint)
    if month: query = query.filter(extract('month', VehicleLog.timestamp) == month)
    if year: query = query.filter(extract('year', VehicleLog.timestamp) == year)
    if week: query = query.filter(func.date_part('week', VehicleLog.timestamp) == week)
    if day: query = query.filter(func.to_char(VehicleLog.timestamp, 'Day').ilike(f'%{day.capitalize()}%'))
    if hour is not None: query = query.filter(extract('hour', VehicleLog.timestamp) == hour)
    if specific_date:
        try:
            dt = datetime.strptime(specific_date, '%Y-%m-%d').date()
            query = query.filter(func.date(VehicleLog.timestamp) == dt)
